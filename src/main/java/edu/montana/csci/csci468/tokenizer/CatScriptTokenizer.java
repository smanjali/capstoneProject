...
public class CatScriptTokenizer {
    ...
    private boolean scanString() {
        if(isQuotation(peek())) {
            takeChar();
            int start = position;
            if(isAlpha(peek()) || isQuotation(peek())) {
                while (isAlpha(peek())) {
                    takeChar();
                    if(isEscaped(peek())) {
                        if (tokenizationEnd()) {
                ...
                if (isQuotation(peek())) {
                    tokenList.addToken(STRING, src.substring(start, position), start, position, line, lineOffset);
                    takeChar();
                } else {
                    tokenList.addToken(ERROR, "<Expected Token: [ \" ]>", start, position, line, lineOffset);
                }
            } else {
                position--;
                return false;
    ...
    //===============================================================
    // Utility functions created by me
    //===============================================================
